...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ezcoin: starting difficulty is 1 / 2^12
...
{
    // Ezcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ezcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Ezcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EzcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EzcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EzcoinMiner(CWallet *pwallet)
{
    printf("EzcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running EzcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EzcoinMiner, pwallet));
}
